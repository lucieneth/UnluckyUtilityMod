...
| `MinecraftAccessor` | `Minecraft` | `@Mutable @Accessor user` + `profileFuture` | Alt switcher: swap the live session with no restart. **Both** must change — `getGameProfile()` reads `profileFuture` first, so a user-only swap joins servers with the new token but the old uuid. |
...
| `PlayerTabOverlayMixin` | `PlayerTabOverlay` | `getNameForDisplay` RETURN | Friends: wraps the returned Component with a `•` prefix (friend blue / self green via `Friends.tablistDotColor`). That method is the single source for the shown name (called for both column-width measurement and drawing), so layout stays consistent. |
...
| `alts/` | **Alt account switcher** (PandoraLauncher-referenced, plan.md Phase 14): `AltAccount` (Microsoft w/ MSA refresh token, or offline username), `AltManager` → `config/unlucky/alts.json` (**sensitive** — MS tokens; default Azure client id embedded, overridable), `MicrosoftAuth` (device-code OAuth → Xbox → XSTS → MC token → profile; user signs in on Microsoft's page, no passwords in-code; refresh-token silent re-auth), `AccountSwitcher` (swaps `user`+`profileFuture` via `MinecraftAccessor`, blocks mid-multiplayer). UI in `gui/alts/` — title-screen right panel (zombie/first-alt preview) + `AltsScreen`. |
